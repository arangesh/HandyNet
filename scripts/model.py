...
def distance_graph(boxes, depth_im):
    """Computes 3d distance of each box from steering wheel
    boxes: [N, (y1, x1, y2, x2)]
    depth_im: [512, 512] depth input image
    """
    steering_wheel_3d = tf.constant([-0.2, 0.2, 1.0])
    depth_im = tf.exp(tf.add(depth_im / 144.0, 6.756)) / 1000.0

    y1, x1, y2, x2 = tf.split(boxes, 4, axis=1)
    x = tf.to_int32(512 * (x1 + x2) / 2)
    y = tf.to_int32(512 * (y1 + y2) / 2)
    z = tf.expand_dims(tf.gather_nd(depth_im, tf.concat([y, x], axis=1)), axis=1)

    x_3d = tf.divide(tf.subtract(tf.to_float(x), 325.5824) * z, 518.8579)
    y_3d = tf.divide(tf.subtract(253.7362, tf.to_float(y)) * z, 519.4694)
    point_3d = tf.concat([x_3d, y_3d, z], axis=1)

    distances = tf.norm(tf.subtract(point_3d, steering_wheel_3d), axis=1, keep_dims=True)

    return (distances <= 0.5 + np.random.exponential(scale=1.7))



def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, input_image, config):
    ...
    input_image: Input depth image given to the network.
    ...
    if config.USE_SMART_TRAIN_ROIS:
        distance_bool = distance_graph(proposals, input_image)
        negative_indices = tf.where(tf.logical_and(tf.logical_and(roi_iou_max < 0.5, no_crowd_bool), distance_bool))[:,
                           0]
    else:
...
class DetectionTargetLayer(KE.Layer):
    ...
    input_images: Input depth images
    ...
    def call(self, inputs):
        ...
        gt_boxes = inputs[2]
        gt_masks = inputs[3]
        input_images = inputs[4]

        # Slice the batch and run a graph for each slice
        # TODO: Rename target_bbox to target_deltas for clarity
        names = ["rois", "target_class_ids", "target_bbox", "target_mask", "input_image"]
        outputs = utils.batch_slice(
            [proposals, gt_class_ids, gt_boxes, gt_masks, input_images],
            lambda w, x, y, z, im: detection_targets_graph(
                w, x, y, z, im, self.config),
...
class expand_roi(KE.Layer):
    def __init__(self, config, **kwargs):
        super(expand_roi, self).__init__(**kwargs)
        self.config = config    
    ...
    def call(self, rois):
        ...
        h = y2 - y1
        w = x2 - x1
        y1 = y1 - self.config.ROI_EXPANSION_FACTOR*h
        x1 = x1 - self.config.ROI_EXPANSION_FACTOR*w
        y2 = y2 + self.config.ROI_EXPANSION_FACTOR*h
        x2 = x2 + self.config.ROI_EXPANSION_FACTOR*w
        return tf.concat([y1, x1, y2, x2], axis=2)


def fpn_classifier_graph(rois, feature_maps,
                         image_shape, pool_size, num_classes):
    """Builds the computation graph of the feature pyramid network classifier head.
...
def fpn_regressor_graph(rois, feature_maps,
                         image_shape, pool_size, num_classes):
    """Builds the computation graph of the feature pyramid network regressor head.
    ...
    # Shape: [batch, num_boxes, pool_height, pool_width, channels]
    x = PyramidROIAlign([pool_size, pool_size], image_shape,
                        name="roi_align_regressor")([rois] + feature_maps)
    # Two 1024 FC layers (implemented with Conv2D for consistency)
    x = KL.TimeDistributed(KL.Conv2D(1024, (pool_size, pool_size), padding="valid"),
                           name="mrcnn_regress_conv1")(x)
    x = KL.TimeDistributed(BatchNorm(axis=3), name='mrcnn_regress_bn1')(x)
    x = KL.Activation('relu')(x)
    x = KL.TimeDistributed(KL.Conv2D(1024, (1, 1)),
                           name="mrcnn_regress_conv2")(x)
    x = KL.TimeDistributed(BatchNorm(axis=3),
                           name='mrcnn_regress_bn2')(x)
    x = KL.Activation('relu')(x)

    shared = KL.Lambda(lambda x: K.squeeze(K.squeeze(x, 3), 2),
                       name="pool_squeeze_regress")(x)
...
class MaskRCNN():
        ...
        if mode == "training":
            ...
            rois, target_class_ids, target_bbox, target_mask =\
                DetectionTargetLayer(config, name="proposal_targets")([
                    target_rois, input_gt_class_ids, gt_boxes, input_gt_masks, KL.Lambda(lambda x: x[..., 0])(input_image)])
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        layer_regex = {
            # Just backbone
            "backbone": r"(conv1)|(bn_conv1)|(res2.*)|(res3.*)|(bn3.*)|(res4.*)|(bn4.*)|(res5.*)|(bn5.*)|(rpn\_.*)|(fpn\_.*)",
